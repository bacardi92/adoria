...
window.onscroll=lazy_load_proc;window.onresize=lazy_load_proc;lazy_load_proc();;(function($){$.fn.drawCSSBipolarChart=function(options){var defaults=$.extend({colors:{left:'#61dac6',right:'#2E6276'},simpleBar:false,bothSides:false,bipolar:false,data:{},chartContainer:'<div class="chart-horiz-container"></div>',ul:'<ul class="chart-horiz"></ul>',li:'<li></li>',divPos:'<div class="posValue"></div>',divNeg:'<div class="negValue"></div>',spanBar:'<div class="bar"></div>',spanValue:'<div class="number-label"></div>',spanValueLabel:'<div class="valueLabel"></div>',transitionDelay:3000},options);return this.each(function(){var maxValue=0,percentage,percentLeft,percentRight,data,i,$ul,$li,$chartContainer;$(this).empty();data=defaults.data;$ul=$(defaults.ul);if(defaults.bothSides){defaults.bipolar=true;}
for(i=0;i<data.length;i++){$li=$(defaults.li);var $spanValueNeg=$(defaults.spanValue).text(data[i][0]);var $spanValuePos=$(defaults.spanValue).text(data[i][1]);var $divNeg=$(defaults.divNeg).append(defaults.spanBar).append($spanValueNeg);var $divPos=$(defaults.divPos).append(defaults.spanBar).append($spanValuePos);if(defaults.simpleBar){var $spanLabel=$(defaults.spanValue).text(data[i][0]);$divPos.append($spanLabel);$divPos.find(".number-label:first").addClass("label-left")
$divNeg.hide();$divPos.css({"background":"none","float":"left"});}
$li.append($divNeg).append($divPos);$ul.append($li);}
$chartContainer=$(defaults.chartContainer).append($ul);$(this).append($chartContainer);for(var j=0;j<data.length;j++){if(data[j][1]>maxValue){maxValue=data[j][1];}}
i=0;$chartContainer.find('ul').children().each(function(){var value;value=defaults.simpleBar?data[i][1]:data[i][2];var percentage=Math.round(value*100)+'%';var otherPercentage=(100-Math.round(value*100))+'%';value=Math.round(value*100)/100;value=value==1?0.99:value;value=value==0?0.01:value;if(defaults.simpleBar){var barWidthPercentage=(Math.round(value*100)/maxValue)+'%';$(this).find('.posValue .bar').css("background",defaults.colors.right).animate({'width':barWidthPercentage},defaults.transitionDelay).text(percentage);$(this).find(".posValue .number-label:first").text(percentage).animate({'left':barWidthPercentage},defaults.transitionDelay);}
function renderChart(container,percentile,color,side,average,traitScore,hoverValue){var numberLabel,animateDirection,animateOption={};var ValBar;var hoverHTML='<div class="hoverValue"></div>';if(side=="left"){ValBar=container.find('.negValue .bar');numberLabel=container.find(".negValue .number-label:first");}else{ValBar=container.find('.posValue .bar');numberLabel=container.find(".posValue .number-label:first");}
ValBar.append(defaults.spanValueLabel);animateDirection=side=="left"?animateOption.right=percentile:animateOption.left=percentile;ValBar.find('.valueLabel').addClass(side).text(traitScore).css("border-color",color).animate(animateOption,defaults.transitionDelay);ValBar.css("background",color).animate({'width':percentile},defaults.transitionDelay);ValBar.append($(hoverHTML).text(hoverValue))
numberLabel.css({"color":color,"font-weight":600});}
if(defaults.bipolar){var traitScore=(value*100).toFixed(0)+"%";traitScore=traitScore=="100%"?"99%":traitScore;var hoverValue;if(value==0.5){renderChart($(this),"1%",defaults.colors.right,"left",true,"","average");renderChart($(this),"1%",defaults.colors.left,"right",true,traitScore,"average");}else if(value<0.5){hoverValue=Math.abs((1-value)*100)+"%";percentLeft=Math.abs(((value-0.5)*2)*100);renderChart($(this),percentLeft+"%",defaults.colors.right,"left",false,traitScore,hoverValue);}else if(value>0.5){percentRight=Math.abs(100*((value-0.5)*2))+"%";hoverValue=traitScore;renderChart($(this),percentRight,defaults.colors.left,"right",false,traitScore,hoverValue);}}
i++;});});};}(jQuery));(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else if(typeof module==='object'&&module.exports){module.exports=function(root,jQuery){if(jQuery===undefined){if(typeof window!=='undefined'){jQuery=require('jquery');}else{jQuery=require('jquery')(root);}}
factory(jQuery);return jQuery;};}else{factory(jQuery);}})(function($){function CircleProgress(config){this.init(config);}
...
initialConfig.animation=JSON.parse(initialConfig.animation);config=$.extend(initialConfig,config);config.el=el;instance=new CircleProgress(config);el.data(dataName,instance);}});};});var Adoria={"adoria_clients_europe":"20","adoria_clients_australia":"22","adoria_clients_usa":"58","adoria_employees_phd":"98","adoria_employees_fluent_english":"100"};$(document).ready(function(){var el=$('#employees'),el2=$('#s_circle'),el3=$('#locations'),inited=false;el.appear({force_process:true});el2.appear({force_process:true});el3.appear({force_process:true});var sampleData1=[["Europe",Adoria.adoria_clients_europe/100],["Australia",Adoria.adoria_clients_australia/100],["USA",Adoria.adoria_clients_usa/100]];el.one('appear',function(){if(!inited){el.circleProgress({startAngle:-Math.PI/4*8,value:Adoria.adoria_employees_phd/100,size:150,fill:{color:"#56c4e3"}}).on('circle-animation-progress',function(event,progress){$(this).find('strong').html(parseInt(Adoria.adoria_employees_phd*progress)+'<i>%</i>');});}});el2.one('appear',function(){if(!inited){el2.circleProgress({startAngle:-Math.PI/4*8,value:Adoria.adoria_employees_fluent_english/100,size:150,fill:{color:"#50c788"}}).on('circle-animation-progress',function(event,progress){$(this).find('strong').html(parseInt(Adoria.adoria_employees_fluent_english*progress)+'<i>%</i>');});}});el3.one('appear',function(){if(!inited){el3.drawCSSBipolarChart({simpleBar:true,bothSides:false,bipolar:false,data:sampleData1,colors:{right:'#90caf9'}});}});$('.slider-testimonial').slick({slidesToShow:1,slidesToScroll:1,arrows:false,fade:true,speed:300,autoplay:true,asNavFor:'.slider-testimonial-nav',responsive:[{breakpoint:600,settings:{autoplay:true,}},{breakpoint:480,settings:{autoplay:true,}},{breakpoint:380,settings:{autoplay:true,}}]});$('.slider-testimonial-nav').slick({slidesToShow:6,slidesToScroll:1,asNavFor:'.slider-testimonial',dots:false,centerMode:true,autoplay:true,focusOnSelect:true,responsive:[{breakpoint:600,settings:{autoplay:false,slidesToShow:3,slidesToScroll:1}},{breakpoint:480,settings:{autoplay:false,slidesToShow:3,slidesToScroll:1}},{breakpoint:380,settings:{autoplay:false,slidesToShow:2,slidesToScroll:1}}]});$(".scrollToIndustry").on('click',function(e){e.preventDefault();$('html,body').animate({scrollTop:$("#industrySection").offset().top-40},600);});});
