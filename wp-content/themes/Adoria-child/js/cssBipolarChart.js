    ...
    return this.each(function() {
      ...
      for ( i = 0; i < data.length; i++) {
        ...
        if (defaults.simpleBar) {
          ...
          $divPos.find(".number-label:first").addClass("label-left")
          // .css("left", 0).empty();
          $divNeg.hide();
          $divPos.css({
            "background": "none",
      ...
      $chartContainer.find('ul').children().each(function() {
        ...
        if (defaults.simpleBar) {
          ...
          $(this).find('.posValue .bar').css("background", defaults.colors.right).animate({'width': barWidthPercentage}, defaults.transitionDelay).text(percentage);
